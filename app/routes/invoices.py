...
def get_invoices():
    ...
    try:
        ...
        for inv in invoices:
            ...
            if inv_dict.get('paid_amount'):
                inv_dict['paid_amount'] = float(inv_dict['paid_amount'])
            if inv_dict.get('batch_amount'):
                inv_dict['batch_amount'] = float(inv_dict['batch_amount'])
            
            # Parse items JSON for GST/TCS
...
def create_invoice():
    ...
    # Calculate GST and TCS if provided
    ...
    # Store tax details in items JSON
    items_data = {
        'base_amount': amount,
